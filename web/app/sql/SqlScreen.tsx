...
const SqlScreen = () => {
  const initialTabs = typeof window !== "undefined" && localStorage.getItem("sql_tabs")
    ? JSON.parse(localStorage.getItem("sql_tabs") as string)
    : [{ id: 1, title: "Query 1" }];

  const initialActiveTabId = typeof window !== "undefined" && parseInt(localStorage.getItem("sql_activeTabId") as string) || 1;
  const initialSelectedSql = typeof window !== "undefined" && localStorage.getItem(`sql_${initialActiveTabId}`) || "SELECT now();";
  ...
  const [maxTabId, setMaxTabId] = useState(initialTabs.length > 0 ? Math.max(...initialTabs.map(tab => tab.id)) : 1);
    ...
    if (id === activeTabId) {
      if (newTabs.length > 0) {
        ...
        if (typeof window !== "undefined") {
          ...
          setSelectedSql(localStorage.getItem(`sql_${newTabs[0].id}`) || "SELECT now();");
        }
      } else {
        addTab();
      }
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      setSelectedSql(localStorage.getItem(`sql_${activeTabId}`) || "SELECT now();");
    }
  }, [tabs, activeTabId]);
